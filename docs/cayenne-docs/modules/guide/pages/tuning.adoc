...
`xref:queries.adoc#select[ObjectSelect]` query supports all three types of semantics. You can mix and match them in the same query for
different prefetches.

`xref:queries.adoc#sqlselect[SQLSelect]` query supports "JOINT" and "DISJOINT_BY_ID". It does not work with "DISJOINT", as the query does not provide
...
`xref:queries.adoc#ejbql[EJBQLQuery]` uses the "FETCH" keyword to enable prefetching:
...
It is possible to fetch specific attributes and relationships from a model using xref:queries.adoc#ejbql[EJBQLQuery].
...
Use can use xref:jCache.adoc#ext-jcache[JCache integration module] to enable any of JCache API compatible caching providers.
...
This can be done with the help of xref:cacheInvalidation.adoc#ext-cache-invalidation[Cache invalidation extension].
