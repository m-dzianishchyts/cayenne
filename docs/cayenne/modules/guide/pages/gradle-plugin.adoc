...
Cayenne Gradle plugin provides tasks similar to xref:maven-plugin.adoc[Maven plugin]. It also provides `cayenne` extension that
...
buildscript {
    ...
    dependencies {
        classpath group: 'org.apache.cayenne.plugins', name: 'cayenne-gradle-plugin', version: '{project-version}'
    }
}
...
dependencies {
    compile 'org.apache.cayenne:cayenne:{project-version}'
}
----
...
as in xref:maven-plugin.adoc#cdbimportTable[Maven Plugin], with the exception that Gradle version will use Groovy instead of XML. Here is
...
xref:maven-plugin.adoc#cdbgenTable[Maven plugin]. Here is example of how to configure default `cdbgen` task:
