...
class Oracle8LOBBatchAction implements SQLAction {
    ...
    public void performAction(Connection connection, OperationObserver observer) throws SQLException, Exception {
        ...
        int updated;
        String updateStr = translator.getSql();
        ...
        // 2. run LOB update (SELECT...FOR UPDATE and writing out LOBs)
        processLOBQuery(connection, translator, selectQuery, qualifierAttributes);

        // finally, notify delegate
        observer.nextCount(query, updated);
    }

    void processLOBQuery(Connection connection, Oracle8LOBBatchTranslator<?> queryBuilder,
                         Oracle8LOBBatchQueryWrapper batchQuery, List<DbAttribute> qualifierAttributes)
            throws SQLException {

        List<DbAttribute> lobAttributes = batchQuery.getDbAttributesForUpdatedLOBColumns();
        if (lobAttributes.isEmpty()) {
            return;
        }

        List<Object> lobValues = batchQuery.getValuesForUpdatedLOBColumns();
        int lobSize = lobAttributes.size();

        String selectStr = queryBuilder.createLOBSelectString(lobAttributes, qualifierAttributes);
        try (PreparedStatement selectStatement = connection.prepareStatement(selectStr)) {
